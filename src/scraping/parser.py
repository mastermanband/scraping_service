	...
	if url:
		if resp.status_code == 200:
			...
			if main_list:
				...
				errors.append({'url': url, 'title': "list does not exists"})
		else:
			errors.append({'url': url, 'title': "Page do not response"})
	return jobs, errors


def work_city(url, city=None, language=None):
	resp = requests.get(url, headers=header[randint(0, 2)])
	jobs = []
	errors = []
	if url:
...
'''def super_job(url):
	...
	if resp.status_code == 200:
		soup = Bs(resp.content, 'html.parser')
		main_mass = soup.find('div', attrs={'class': '_1ID8B'})
		if main_mass:
			mass_list = main_mass.find_all('div', class_='Fo44F QiY08 LvoDO')
			for mass in mass_list:
				title = mass.find('div', class_='_3mfro PlM3e _2JVkc _3LJqf')
				title_url = title.a['href']
				description = mass.find('div', class_='HSTPm _3C76h _10Aay _2_FIo _1tH7S').text
				company = mass.find('span', class_='_3mfro _3Fsn4 f-test-text-vacancy-item-company-name _9fXTd _2JVkc _2VHxz _15msI').text
				jobs.append({
					'title': title.text, 'url': domain + title_url, 'description': description, 'company': company,
				})
		else:
			errors.append({'url': url, 'title': "mass does not exists"})
	else:
		errors.append({'url': url, 'title': "Page do not response"})
	return jobs, errors'''
